...
def scan_tickets():
    ...
    if request.method == "POST":
        ...
        all_active_book_ids = check_error(
            DatabaseQueries.get_all_active_book_ids(db=db_path), message_holder=msg_data
        )
        ...
        if extracted_vals == "INVALID BARCODE":
            ...
            if check_error(
                DatabaseQueries.is_counting_ticket_number_set(
                ...
                msg_data["message"] = (
                    "A ticket from this book has already been scanned. Please use the UNDO button if you want to rescan.".upper()
                )
                msg_data["message_type"] = "error"
                return render_template(
                    "scan_tickets.html",
                    activated_books=check_error(
                        DatabaseQueries.get_scan_ticket_page_table(db=db_path),
                        message_holder=msg_data,
                    ),
                    instant_tickets_sold_total=check_error(
                        calculate_instant_tickets_sold(ReportID="Pending"),
                        message_holder=msg_data,
                    ),
                    counting_order=load_config()["ticket_order"],
                    activated_book_count=check_error(
                        DatabaseQueries.count_activated_books(db_path),
                        message_holder=msg_data,
                        fallback=0,
                    ),
            ...
            TicketName = check_error(
                DatabaseQueries.get_ticket_name(db_path, extracted_vals["game_number"]),
                message_holder=msg_data,
            )
            check_error(
                insert_ticket(
            ...
            check_error(
                Database.update_counting_ticket_number(
    ...
    return render_template(
        ...
        instant_tickets_sold_total=check_error(
            calculate_instant_tickets_sold(ReportID="Pending"), message_holder=msg_data
        ),
        counting_order=load_config()["ticket_order"],
        activated_book_count=check_error(
            DatabaseQueries.count_activated_books(db_path),
...
def book_sold_out():
    ...
    try:
        ...
        book = check_error(
            DatabaseQueries.get_book(db_path, book_id), message_holder=msg_data
        )
        game_number = book[1]
        book_amount = book[3]
        TicketPrice = book[4]
