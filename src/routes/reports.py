...
def edit_reports():
    ...
    sales_reports = check_error(
        DatabaseQueries.get_all_sales_reports(db_path), msg_data, fallback=[]
    )
...
@report_bp.route(
    "/edit_report/<report_id>/<updated_report_ids>", methods=["GET", "POST"]
)
def edit_single_report(report_id, updated_report_ids=None):
    ...
    sales_logs = check_error(
        DatabaseQueries.get_sales_log(db_path, report_id),
        message_holder=msg_data,
        fallback=[],
    )
    sale_report = check_error(
        DatabaseQueries.get_daily_report(db_path, report_id),
        message_holder=msg_data,
        fallback={},
    )
    ...
    if sale_report:
        instant_tickets_sold_total = check_error(
            calculate_instant_tickets_sold(ReportID=report_id), msg_data, fallback=0
        )
...
def update_sales_log():
    ...
    try:
        ...
        latest_reportID = (
            int(check_error(lambda: DatabaseQueries.next_report_ID(db_path), msg_data))
            - 1
        )

        game_number = check_error(
            DatabaseQueries.get_game_num_of(db_path, book_id), msg_data
        )
        ...
        TicketName = check_error(
            DatabaseQueries.get_ticket_name(db_path, game_number), msg_data
        )
        book_amount = book[3]
        TicketPrice = book[4]
        ...
        check_error(
            Database.update_sales_log_current_TicketNum(
                db_path, close, report_id, book_id
            ),
            msg_data,
        )
        ...
        if is_book_sold and close != "-1":
            ...
            check_error(
                Database.update_is_sold_for_book(db_path, False, book_id), msg_data
            )
            ...
            for id in range(report_id_int + 1, latest_reportID + 1):
                ...
                check_error(
                    insert_ticket(
                        scanID, book_id, close, TicketName, TicketPrice, str(id)
                    ),
                    msg_data,
                )
        ...
        if not previous_reportID < 1:
            check_error(
                Database.update_ticketTimeline_ticketnumber(
                    db_path, previous_reportID, book_id, open
                ),
                msg_data,
            )
            check_error(
                Database.update_sales_log_current_TicketNum(
                    db_path, open, previous_reportID, book_id
                ),
                msg_data,
            )
            prev_instant_sold = check_error(
                calculate_instant_tickets_sold(previous_reportID), msg_data
            )
            check_error(
                Database.update_sale_report_instant_sold(
                    db_path, prev_instant_sold, previous_reportID
                ),
                msg_data,
            )
        ...
        if next_reportID <= latest_reportID:
            check_error(
                Database.update_sales_log_prev_TicketNum(
                    db_path, close, next_reportID, book_id
                ),
                msg_data,
            )
            next_instant_sold = check_error(
                calculate_instant_tickets_sold(next_reportID), msg_data
            )
            check_error(
                Database.update_sale_report_instant_sold(
                    db_path, next_instant_sold, next_reportID
                ),
                msg_data,
            )
        ...
        if latest_reportID == report_id_int:
            check_error(
                Database.update_isAtTicketNumber_val(db_path, book_id, close), msg_data
            )
        ...
        check_error(
            Database.update_ticketTimeline_ticketnumber(
                db_path, report_id, book_id, close
            ),
            msg_data,
        )
        ...
        return jsonify(
            {
                "redirect_url": url_for(
                    ...
                    report_id=report_id,
                    updated_report_ids=updated_report_ids.__str__(),
                    message=msg_data.get("message", ""),
                    message_type=msg_data.get("message_type", ""),
                )
            }
