...
def calculate_instant_tickets_sold(ReportID):  # TODO: RETURNING ERROR MESSAGE
    ...
    instant_tickets_sold_quantanties = check_error(
        DatabaseQueries.get_all_instant_tickets_sold_quantity(db_path, ReportID),
        msg_data,
        fallback=[],
    )
...
def create_daily_invoice(ReportID, return_path_only=False):
    msg_data = {"message": "", "message_type": ""}
    config = load_config()
    invoiceLog = check_error(
        DatabaseQueries.get_table_for_invoice(db_path, ReportID), msg_data
    )
    ...
    store_name = (
        "Store Name"
        if load_config()["business_name"] is None
        else load_config()["business_name"]
    )
    address = (
        "Store Address"
        if load_config()["business_address"] is None
        else load_config()["business_address"]
    )
    phone = (
        "N/a"
        if load_config()["business_phone"] is None
        else load_config()["business_phone"]
    )
    email = (
        "N/a"
        if load_config()["business_email"] is None
        else load_config()["business_email"]
    )
    ...
    daily_report = check_error(
        DatabaseQueries.get_daily_report(db_path, ReportID), msg_data
    )
    ...
    try:
        generate_invoice.generate_lottery_invoice_pdf(
            full_path, store_info, invoiceLog, invoice_number, daily_report
        )
...
def add_sales_log(book_id, lastest_ticket_number, game_number):
    ...
    activate_book_isAtTicketNumber = check_error(
        DatabaseQueries.get_activated_book_isAtTicketNumber(db_path, book_id),
        message_holder=msg_data,
    )
    ticket_name = check_error(
        DatabaseQueries.get_ticket_name(db_path, game_number), message_holder=msg_data
    )
    ...
    check_error(
        Database.insert_sales_log(db_path, sale_log_info), message_holder=msg_data
    )
...
def do_submit_procedure():
    msg_data = {"message": "", "message_type": ""}
    try:
        next_ReportID = check_error(
            DatabaseQueries.next_report_ID(db_path), msg_data
        )  # STRING
        # Get form values
        daily_totals = {
        ...
        sold_out_books = check_error(
            DatabaseQueries.get_all_sold_books(db_path, next_ReportID), msg_data
        )
