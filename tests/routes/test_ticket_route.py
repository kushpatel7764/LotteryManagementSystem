...
def test_scan_invalid_barcode(mock_scan_mgr, mock_active_ids, mock_flash, client, auth):
    ...
    resp = client.post(
        "/scan_tickets", data={"scanned_code": "123456"}, follow_redirects=False
    )
...
def test_scan_book_not_activated(
    ...
    mock_flash.assert_any_call(
        "Book IS NOT ACTIVATED! PLEASE ACTIVATE BEFORE SCANNING.", "tickets_error"
    )
...
def test_book_sold_out_success(
    ...
    resp = client.post("/book_sold_out", data={"book_id": "B1"}, follow_redirects=True)
